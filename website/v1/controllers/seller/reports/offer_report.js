...
const completed_offer_report = async (req, res, next) => {
	let { start_date, end_date, search, product_variant, category_id, percent_start, percent_end, limit, page } = req.body;
	...
	try {
		...
		for (let index = 0; index < offer.length; index++) {
			...
			excelExportData.push({
				...
				OfferDiscountAmount: element.offerDiscountAmount,
				CouponDiscountAmount: element.couponDiscountAmount,
				ProductVariantNetPrice: element.productVariantNetPrice,
				GrossSale: element.grossSale,
				GrossSale: element.grossSale,
				TotalDiscountAmount: element.totalDiscountAmount,
				TotalAmount: element.totalAmount,
				AverageDiscountPercentage: element.averageDiscountPercentage,
		...
		var down = `${folder}offer_report_Export_${today.toDateString()}_${today.getHours()}${today.getMinutes()}${today.getSeconds()}.xlsx`;
...
const product_variant_list = async (req, res) => {
...
module.exports = { completed_offer_report, completed_offer_report_excel, product_variant_list };
