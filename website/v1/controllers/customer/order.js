...
let { receipt } = require('./../../middlewares/receiptTemplate');
    ...
    try {
        ...
        let addressDetails = {
            billingDetails: {
                ...
                blockNumber: billing['blockNumber'] || null,
                city: billing['city'],
                state: billing['state'],
            ...
            shippingDetails: {
                ...
                blockNumber: shipping['blockNumber'] || null,
                city: shipping['city'],
        ...
        //step 1 - check if this is first order of customer or not.
        //step 2 - check if customer has referredBy or not if not then skip step 3 else go to step 3
        //step 3 - check setting referal status if(status == true) then get referalPercentage else referalPercentage = 0

        let referalPercentage = 0;
        let orderCheckForRefer = await allModels.orderModel.find({ "customerId": req.userId })

        if (orderCheckForRefer.length === 0) {
            let checkCustomerRefferedBy = await allModels.customer.findOne({ _id: req.userId })
            if (checkCustomerRefferedBy.referredBy != null && checkCustomerRefferedBy.referredBy != "") {
                let checkSettingReferal = await allModels.settingModel.findOne({ "referral.status": true })
                if (checkSettingReferal) {
                    referalPercentage = parseFloat(checkSettingReferal.referral.referalPercent.toString())
                }
            }
        }
        ...
        //step 1 - if referalPercentage > 0 then get referedBy from customer table and also get customerId
        //step 2 - fetch data from orderItem of current orderID (newOrder._id) and calculate total price of current order
        //step 3 - add fund to customer wallet (customerID from step 1)

        try {
            if (referalPercentage > 0) {
                let checkCustomerRefferedBy = await allModels.customer.findOne({ _id: req.userId })
                let parentReferal = await allModels.customer.findOne({ referralCode: checkCustomerRefferedBy.referredBy });
                let orderItems = await allModels.orderItems.find({ orderId: newOrder._id });
                let totalOrderAmt = 0;
                for (let i = 0; i < orderItems.length; i++) {
                    totalOrderAmt += parseFloat(orderItems[i].grandTotal.toString())
                }
                totalOrderAmt = parseFloat(((totalOrderAmt / 100) * referalPercentage).toFixed(3))

                //add totalOrderAmt to wallet of parentReferal
                let walletData = await allModels.walletModel.find({ customerId: parentReferal._id });
                let currentBalance = 0;
                if (walletData.length > 0) {
                    let a = await allModels.walletModel.find({ customerId: parentReferal._id }).sort({ _id: -1 }).limit(1);
                    currentBalance = parseFloat(a[0].currentBalance);
                }

                let wallet = new allModels.walletModel({
                    customerId: parentReferal._id,
                    transactionType: 'credit',
                    creditAmount: totalOrderAmt,
                    fundBy: { id: req.userId, userType: "Customer" }, //need to update with adminId (MYMARKET ID)
                    fundReason: `Referal Commission`,
                    fundRemarks: `Referal Commission on order ${newOrder._id}`,
                    currentBalance: parseFloat(currentBalance + parseFloat(totalOrderAmt)).toFixed(3)
                });
                await wallet.save()
            }
        } catch (err) { }
        ...
        for (let i = 0; i < data.productVariants.length; i++) {
            ...
            try {
                ...
                if (parseFloat(pvariant.offerProductPrice).toString().toLowerCase() != 'nan') {
                    offerDiscount = parseFloat(product['productNetPrice']) - parseFloat(pvariant.offerProductPrice)
                }
                ...
                if (!checkShiping) {
                    ...
                    let orderStatus = await allModels.orderStatusUpdate({
                        ...
                        description: "New order from website",
                        indexNo: lastOrderStatusIndex.indexNo + 1
                    })
                    await orderStatus.save();
                }

                //notification.addNotification(req.userId, product['sellerId'], "order", `New Order Placed [Product name: ${product['productVariantDetails'][0].productVariantName}]`, "website", orderId)
...
exports.fetchAllOrderByCustomerId = async (req, res) => {
    ...
    const orderShipping = await allModels.orderModel.aggregate([
        ...
        { $addFields: { totalPrice: { $sum: "$orderItems.grandTotal" }, } },
        { $match: status },
...
exports.updateOrder = async (req, res) => {
    let order = await allModels.orderModel.findById({
        _id: req.params.id,
        customerId: req.userId
    });
    //console.log(order)
    if (!order) { return res.status(403).send({ message: "Invalid order id selected" }); }
    order['transactionID'] = req.body.transactionID ? req.body.transactionID : order['transactionID']
    await order.save()
    //console.log(data)
...
exports.orderSearch = async (req, res) => {
    ...
    if (req.body.search.length > 2) {
        ...
        let orderList = [];
        let order = await allModels.orderModel.find({
            customerId: req.userId
        ...
        let asyncOrderForEach = async (orders, orderList) => {
            return new Promise((resolve, reject) => {
                orders.forEach(async (order, i) => {
                    let pvariants = await allModels.orderItems
        ...
        await asyncOrderForEach(order, orderList);
        var RESPONSE_DATA = orderList.filter(a => {
            let variants = a.productVariants.filter(f => {
                return f.productDetails[0].productVariantName.toLowerCase().includes(search.toLowerCase());
            })
            return variants.length > 0;
        });
        return res.send({ count: RESPONSE_DATA.length, data: RESPONSE_DATA });
    } else {
    ...
    try {
        ...
        if (["new", "processing", "on_hold"].includes(orderStatus.status.toLowerCase())) {
            ...
            if (isErrorUpdate) {
                ...
                data = {}
                data.shippingPrice = orderShipping.shippingPrice
                data.customername = req.customer.firstName.toUpperCase()
                data.sellername = orderShipping.sellerId.nameOfBussinessEnglish
                ...
                //Sending Notification
                //For customer
                await sendNotification(req, null, req.userId, '4', data, 'order', data.order._id)
                sendNotification(req, null, orderShipping.sellerId._id, '21', data, 'order', data.order._id)
            ...
            if (orderItemList.length == 0) {
                ...
                for (let index = 0; index < orderItemList.length; index++) {
                    ...
                    if (filter.length == 1) {
                        ...
                        orderItem.ordernumber = order.indexNo + '_' + orderShipping.indexNo
                        orderItem.customername = order.customerId.firstName.toUpperCase()
                        let adminId = "61c961934280680ee8782e76"

                        //Seller Notification
                        await sendNotification(req, null, orderItem.sellerId, '30', orderItem, 'order', orderItem.order._id)
                        //Admin Notification
                        await sendNotification(req, null, adminId, '45', orderItem, 'order', orderItem.order._id)
                        //End Notification Work
                    } else {
                        isErrorUpdate = true
                    }
