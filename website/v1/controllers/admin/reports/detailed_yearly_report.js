...
const detailed_yearly_report = async (req, res, next) => {
	let { limit, page, year } = req.body;
	...
	try {
		...
		const sales = await ALL_MODELS.orderItems.aggregate([
			...
			{ $addFields: { RefundedAmount: { $cond: { if: {$ne:["$RefundedAmount",""]}, then: "$RefundedAmount", else: 0 } } } },
			{
				$addFields: {
					NateSales: {
...
// End of detailed_yearly_report

module.exports = detailed_yearly_report;
